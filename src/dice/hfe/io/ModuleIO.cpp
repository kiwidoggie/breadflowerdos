...
void ModuleIO::getClassList(std::vector<ClassRegInfo>& regInfos)
{
    ...
    /*
    {
        auto regInfo = ClassRegInfo(CID_ControlMap, "dice.hfe.io.ControlMap.Standard", ControlMap::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_NullInputDeviceManager, "dice.hfe.io.InputDeviceManager.Null", NullInputDeviceManager::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_NullResourceManager, "dice.hfe.io.ResourceManager.Null", NullResourceManager::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_NullCacheManager, "dice.hfe.io.CacheManager.Null", NullCacheManager::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_ControlMapManager, "dice.hfe.io.ControlMapManager.Standard", ControlMapManager::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_ExcelLexicon, "dice.hfe.ExcelLexicon", ExcelLexicon::create);
        regInfos.push_back(regInfo);
    }
    */
}
...
bool ModuleIO::init()
{
    ...
    g_mainConsoleObjects->registerTypeConversion("std::string", "string");

    g_system->resetExactTime();

    addConstantToAllConsoles("c_Debug", "0");
    ...
    addConstantHelper("c_GISayTeam", 23);
    ...
    addConstantHelper("c_GILeader", 60);
