...
void SimpleParser::add(
	int32_t key, const std::string& setting, const std::string& description)
{
	m_settings.emplace(key, setting);
	m_descriptions.emplace(key, description);
}

bool SimpleParser::parseString(
	const std::string& input,
	std::list<std::pair<int, std::string>>& outputList)
{
	...
	while (pos != std::string::npos)
	{
		...
		std::string token =
			workingStr.substr(0, pos + 1);	  // +1 to include the delimiter
...
bool SimpleParser::getKey(
	const std::string& input, int32_t* outKey, std::string* outValue)
{
	*outKey = -1;
			...
			*outKey = key;
			...
			if (input.length() > valueStart && input[valueStart] == '"')
			{
				...
				if (endQuote == std::string::npos)
					return false;
				*outValue =
					input.substr(valueStart + 1, endQuote - valueStart - 1);
			}
			else
			{
