...
void BF2Engine::initDefaultSettings()
{
	...
	io::g_settingsManager
		->U32RegisterUserVar("GSPerfLogAtPlayerCount", 30, true, 0);
	io::g_settingsManager
		->boolRegisterUserVar("GSUseObjectCache", true, true, 0);
	io::g_settingsManager
		->U32RegisterUserVar("GSInterpolationTime", 100, true, 0);
	io::g_settingsManager
		->U32RegisterUserVar("GSExtrapolationTime", 1200, true, 0);
	io::g_settingsManager
		->floatRegisterUserVar("GSDefaultLatencyCompensation", 0.1f, true, 0);
	io::g_settingsManager
		->boolRegisterUserVar("GSUseLatencyCompensation", false, false, 0);
	io::g_settingsManager
		->boolRegisterUserVar("GSUseClientSidePrediction", true, false, 0);
	io::g_settingsManager
		->boolRegisterUserVar("GSDebugBitStream", false, false, 0);
	io::g_settingsManager
		->stringRegisterUserVar("GSPlayerName", "DefaultPlayerName", true, 0);
	...
	io::g_settingsManager
		->boolRegisterUserVar("GSExtrapolateFrame", false, true, 0);
	io::g_settingsManager
...
bool BF2Engine::parseParameters(const std::string& launchArgs)
{
	...
	if (!launchArgs.empty())
	{
		...
		{
			simpleParser.add(
				LaunchArgs::LaunchArg_Help,
				"help",
				"Displays this help");
			simpleParser.add(LaunchArgs::LaunchArg_Help2, "?", "Same as +help");
			simpleParser.add(
				LaunchArgs::LaunchArg_Dedicated,
				"dedicated",
				"Start in dedicated server mode");
			simpleParser.add(
				LaunchArgs::LaunchArg_Multi,
				"multi",
				"Allow starting multiple BF2 instances");
			simpleParser.add(
				LaunchArgs::LaunchArg_Config,
				"config",
				"Sets path to the ServerSettings.con file to use");
			simpleParser.add(
				LaunchArgs::LaunchArg_RSConfig,
				"rsconfig",
				"Sets path to the ReservedSlots.con file to use");	  // only
																	  // bf2
			simpleParser.add(
				LaunchArgs::LaunchArg_MapList,
				"mapList",
				"Sets the path to the MapList.con file to use");
			simpleParser.add(
				LaunchArgs::LaunchArg_PbPath,
				"pbPath",
				"Set the path to use for PunkBuster on multi-instance "
				"configurations (defaults to {install_dir}/pb");
			simpleParser.add(
				LaunchArgs::LaunchArg_JoinServer,
				"joinServer",
				"Join a server by ip address or hostname");
			simpleParser.add(
				LaunchArgs::LaunchArg_Port,
				"port",
				"specifies the network port to be used");
			simpleParser.add(
			...
			simpleParser.add(
				...
				"modPath",
				"Set the mod path (default mods/bf2142)");
#else
			simpleParser.add(
				LaunchArgs::LaunchArg_ModPath,
				"modPath",
				"Set the mod path (default mods/bf2)");
#endif
			simpleParser.add(
				LaunchArgs::LaunchArg_CheckForAvailablePatch,
				"checkForAvailablePatch",
				"");	// only bf2
			simpleParser.add(
				LaunchArgs::LaunchArg_CheckForPatch,
				"checkForPatch",
				"");	// only bf2
			simpleParser.add(LaunchArgs::LaunchArg_UseAI, "ai", "");
			simpleParser.add(
				LaunchArgs::LaunchArg_Fullscreen,
				"fullscreen",
				"Start game in full screen mode");
			simpleParser.add(
				LaunchArgs::LaunchArg_WX,
				"wx",
				"Position game window on the screen at certain x-position");
			simpleParser.add(
				LaunchArgs::LaunchArg_WY,
				"wy",
				"Position game window on the screen at certain y-position");
			simpleParser.add(
				LaunchArgs::LaunchArg_SzX,
				"szx",
				"Set resolution width");
			simpleParser.add(
				LaunchArgs::LaunchArg_SzY,
				"szy",
				"Set resolution height");
			simpleParser.add(
				LaunchArgs::LaunchArg_NoSound,
				"noSound",
				"Start game without sound");
			simpleParser.add(
				LaunchArgs::LaunchArg_Restart,
				"restart",
				"Used when restarting executable.");	// only bf2
			simpleParser.add(
				LaunchArgs::LaunchArg_SkipDXCheck,
				"skipDXCheck",
			...
			simpleParser.add(
				...
				"ranked",
				"Allows gamespy snapshot sending");
			simpleParser.add(
				LaunchArgs::LaunchArg_NoStatusMonitor,
				"noStatusMonitor",
				"Start game without status monitor screen");
			simpleParser.add(
				LaunchArgs::LaunchArg_Demo,
				"demo",
				"Sets the con-file with demo options");
			simpleParser.add(
				LaunchArgs::LaunchArg_LoadLevel,
				"loadLevel",
			...
			simpleParser.add(
				...
				"eaAccountName",
				"Auto-login with the specified EA Account Name");	 // only
																	 // bf2142
			simpleParser.add(
				LaunchArgs::LaunchArg_EAAccountPassword,
				"eaAccountPassword",
				"Password to the specified EA Account Name");	 // only bf2142
			simpleParser.add(
			...
			simpleParser.add(
				...
				"playerPassword",
				"Set the player password");	   // only bf2
			simpleParser.add(
				LaunchArgs::LaunchArg_PlayNow,
				"playNow",
				"use playnow functionality");	 // only bf2
			simpleParser.add(
				LaunchArgs::LaunchArg_HostServer,
				"hostServer",
				"");
			simpleParser.add(
				LaunchArgs::LaunchArg_LowPriority,
				"lowPriority",
				"Run the game with slightly lower priority");
			simpleParser.add(
				LaunchArgs::LaunchArg_GameMode,
				"gameMode",
				"Sets the game mode.");
			simpleParser.add(
				LaunchArgs::LaunchArg_MaxPlayers,
				"maxPlayers",
				"Sets max players.");
			simpleParser.add(
		...
		if (simpleParser.parseString(launchArgs, paramList))
		{
			for (auto& [id, value] : paramList)
			{
				...
				case LaunchArgs::LaunchArg_DeviceBehavior:
					...
					if (deviceBehavior != 32 && deviceBehavior != 64 &&
						deviceBehavior != 80)
					{
						deviceBehavior = 80;
					}
 ...
 g_mapList->m_configFile =
	...
	// TODO: Vars::set("opt-nosound", "1"; // TODO: makes no sense on client.
	// Add server check?
...
// bf2: 004dc0a0
bool BF2Engine::playDemo(bool, const std::string&)
{
	// TODO: Implement
	return true;
}
...
{
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		1,
		"ECCoreCEChangedSetting");
}
