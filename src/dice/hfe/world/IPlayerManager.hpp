...
namespace dice::hfe::world
{
	...
	public:
		...
		virtual IPlayer* getPlayerFromName(const std::string&) = 0;
		...
		virtual IPlayer* getPlayerFromIdString(const std::string&) = 0;
		...
		virtual void getPlayerClasses(
			std::list<std::pair<uint32_t, std::string>>&) const = 0;
		virtual const std::string&
		getClassNameFromPlayerClassID(uint32_t) const = 0;
		virtual IPlayer*
		createPlayer(uint32_t, const std::string&, int32_t) = 0;
		virtual IPlayer*
		createPlayer(const std::string&, const std::string&, int32_t) = 0;
		virtual bool saveAllPlayers(const std::string&) = 0;
