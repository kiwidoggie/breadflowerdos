...
void GameLogicCompModule::getClassList(std::vector<ClassRegInfo>& regInfos)
{
    ...
    /*
    {
        auto regInfo = ClassRegInfo(CID_ArmorGLCompTemplate, "dice.hfe.world.TemplateComponent.Armor", ClassFactory<ArmorGLCompTemplate>::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_AbilityGLCompTemplate, "dice.hfe.world.TemplateComponent.Ability", ClassFactory<AbilityGLCompTemplate>::create);
        regInfos.push_back(regInfo);
    }
    {
        auto regInfo = ClassRegInfo(CID_RadioGLCompTemplate, "dice.hfe.world.TemplateComponent.Radio", ClassFactory<RadioGLCompTemplate>::create);
        regInfos.push_back(regInfo);
    }
    */
}

void GameLogicCompModule::getSingletonList([[maybe_unused]] std::vector<SingletonRegInfo>& regInfos)
{
}
