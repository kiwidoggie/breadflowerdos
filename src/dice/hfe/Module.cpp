...
{
	{
		auto regInfo = ClassRegInfo(
			CID_MemoryPool,
			"dice.hfe.MemoryPool.Standard",
			MemoryPool::create);
	...
	}
	{
		auto regInfo = ClassRegInfo(
			CID_EventManager,
			"dice.hfe.EventManager",
			EventManager::create);
		regInfos.push_back(regInfo);
	}
	{
		auto regInfo =
			ClassRegInfo(CID_Profiler, "dice.hfe.Profiler", Profiler::create);
		regInfos.push_back(regInfo);
	}
	// TODO: Implement
	/*
...
void Module::getSingletonList(std::vector<SingletonRegInfo>& regInfos)
{
	...
	{
		auto regInfo =
			SINGLETON_REG_INFO("System", g_system, IID_IBase, CID_System);
		regInfos.push_back(regInfo);
	}
	/*
	{
...
bool Module::init()
{
	...
	g_eventManager->registerEvent(
		...
		4,
		"ECCoreCEUpdateLoadStats");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		6,
		"ECCoreCEShutdownGame");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		7,
		"ECCoreCEResourcesLoaded");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		8,
		"ECCoreCEDebugTextHook");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		9,
		"ECCoreCECollectSystemInfo");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		13,
		"ECCoreCEUnloadLevel");
	...
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		16,
		"ECCoreCEPeakWinMessages");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		10,
		"ECCoreCEStartPerformanceTest");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		11,
		"ECCoreCEStopPerformanceTest");
	g_eventManager->registerEvent(
		EventCategory::ECCore,
		12,
		"ECCoreCEReportPerformanceInfo");
	return true;
}
